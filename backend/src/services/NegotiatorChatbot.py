...
class Negotiator:
    def __init__(self, crud_service, auth):
        ...
        self.functions_available = { 
                ...
                "propose_payment_plan": self.propose_payment_plan,
                "propose_partial_immediate_payment": self.propose_partial_immediate_payment,
                "identify_arenas_holidays_query": self.identify_arenas_holidays_query,
                "identify_community_holidays_query": self.identify_community_holidays_query,
                "identify_bus_schedule_query": self.identify_bus_schedule_query
            }
        
        self.file_arenas_id = os.getenv("FILE_ID_ARENAS")
        self.file_comunidades_id = os.getenv("FILE_ID_COMUNIDADES")
        self.file_linea105_id = os.getenv("FILE_ID_LINEA105")

        self.system_message = f"""
            Hola, soy Pedro, especialista en ofrecer descuentos por pagos inmediatos y en negociar planes de pagos personalizados, utilizando el euro como moneda. Estoy aquí para asistirte con una variedad de servicios enfocados en la gestión de deudas. Además de mis servicios principales de negociación, tengo acceso a información específica y valiosa que puede ser de interés:

            Festivos de Apertura Comercial del CC Las Arenas (id del documento {self.file_arenas_id}): Tengo información detallada sobre los horarios y los días festivos en los que el Centro Comercial Las Arenas estará abierto. Puedes consultarme si tienes dudas sobre la apertura en fechas particulares, como por ejemplo, "¿El 25 de diciembre está abierto el CC Las Arenas?". Es importante mencionar que si un día festivo no aparece en nuestra lista y coincide con un festivo oficial en Canarias o es un día festivo nacional en España, el centro comercial permanecerá cerrado.

            Festivos de Apertura Comercial por Comunidades Autónomas (id del documento {self.file_comunidades_id}): Dispongo de una base de datos actualizada con los días festivos de apertura comercial permitidos por comunidades autónomas en España, conforme a lo establecido por el BOE (Boletín Oficial del Estado). Esta información te permitirá saber en qué días festivos específicos los centros comerciales están autorizados a abrir en cada comunidad autónoma. Por ejemplo, puedes preguntar: "¿Los centros comerciales en Aragón abren el 25 de diciembre?"
            
            Línea de Guagua 105 Las Palmas de Gran Canaria - Gáldar (id del documento {self.file_linea105_id}): Poseo los horarios completos de la línea de guaguas 105, tanto para días laborales como festivos, cubriendo las rutas de Las Palmas de Gran Canaria a Gáldar y viceversa. Esta información es crucial si necesitas planificar tus viajes y quieres saber, por ejemplo, "¿A qué horas puedo coger la guagua de Gáldar a Las Palmas un domingo?". Ten en cuenta que esta información se limita exclusivamente a la línea 105 entre Las Palmas y Gáldar.
            ...
            Además, mi objetivo no es solo negociaciones de deuda personalizadas y efectivas sino también facilitarte información útil que pueda mejorar tu experiencia y resolver tus dudas específicas. Ya sea que necesites negociar un plan de pagos, obtener un descuento por pago inmediato, o simplemente saber si puedes realizar tus compras en un día festivo, estoy aquí para ayudarte.
    ...
    def identify_arenas_holidays_query(self, year=2022):
        if year != 2022:
            return json.dumps({"error": "Lo siento, solo tengo información sobre los días festivos de apertura de Las Arenas en 2022."})
        
        return json.dumps({"message": f"Los días festivos de apertura de Las Arenas en 2022 se encuentran en el archivo con id: {self.file_arenas_id}. Si no aparece en la lista y el día solicitado es un festivo, el centro comercial estará cerrado."})


    def identify_community_holidays_query(self, year=2023):
        if year != 2023:
            return json.dumps({"error": "Lo siento, solo tengo información sobre los días festivos por comunidades autonomas españolas de 2023."})
        
     
        return json.dumps({"message": f"Los días festivos por provincias españolas de 2023 se encuentran en el archivo con id: {self.file_comunidades_id}."})
    

    def identify_bus_schedule_query(self):

        return json.dumps({"message": f"El horario de la guagua que va de Las Palmas a Gáldar y de Gáldar a Las Palmas se encuentra en el archivo con id: {self.file_linea105_id}."})


    def set_debt_id(self, debt_id):
        self.debt_id = debt_id
        return json.dumps({"message": f"Las negociaciones se realizaran con la deuda de id {debt_id}."})
